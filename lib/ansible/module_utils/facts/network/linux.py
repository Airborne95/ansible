...
class LinuxNetwork(Network):
    ...
    def get_default_interfaces(self, ip_path, collected_facts={}):
        ...
        for v in 'v4', 'v6':
            isV6 = v == 'v6'
            isRedHat = collected_facts.get('ansible_os_family') == 'RedHat'
            startsWith4 = collected_facts.get('ansible_distribution_version', '').startswith('4.')
            rc, out, err = self.module.run_command(command[v], errors='surrogate_then_replace')

            if ((isV6 and isRedHat and startsWith4) or (isV6 and not socket.has_ipv6) or not out):
